...
from agent_operations_viewer.machine_credentials import generate_pairing_session_id
...
from .service_manager import install_service, service_status, start_service, stop_service, uninstall_service
...
def identity_status_payload(settings: Settings, identity: LocalMachineIdentity | None) -> dict[str, object] | None:
    if identity is None:
        return None
    return {
        "machine_id": identity.machine_id,
        ...
        "source_host": identity.source_host,
        "server_base_url": identity.server_base_url,
        "public_key": identity.public_key,
        "paired_at": identity.paired_at,
        "created_by_user_id": identity.created_by_user_id,
        "identity_path": str(machine_identity_path(settings)),
    }
...
def pair_machine(
    ...
    *,
    ...
    if existing_identity is not None and not force:
        raise SystemExit(
            "This machine is already paired. Run `python -m agent_daemon unpair` "
            "or `python -m agent_daemon repair --re-pair` first."
        )
...
def _machine_auth_probe(settings: Settings, identity: LocalMachineIdentity) -> dict[str, object]:
    server_base_url = _clean_url(settings.server_base_url) or _clean_url(identity.server_base_url)
    if not server_base_url:
        return {"ok": False, "error": "No server URL is configured."}
...
def machine_status(settings: Settings) -> dict[str, object]:
    ...
    server_base_url = _clean_url(settings.server_base_url) or (
        _clean_url(identity.server_base_url) if identity is not None else None
    )
    ...
    return {
        ...
        "identity": identity_status_payload(settings, identity),
...
def machine_repair(
    ...
    *,
    ...
    if identity is None and re_pair:
        ...
        raise SystemExit("No machine credential is configured. Run `python -m agent_daemon pair`.")
    ...
    *,
    ...
    if existing_identity is not None and not force:
        if (
            ...
            and existing_identity.server_base_url.rstrip("/") != settings.server_base_url.rstrip("/")
        ):
            raise SystemExit(
                "This machine is already paired to a different server. "
                "Run `python -m agent_daemon setup --force --server URL` to replace the credential."
            )
        pair_result = {
            "paired": True,
            "already_paired": True,
            "identity": identity_status_payload(settings, existing_identity),
        }
    else:
        pair_result = pair_machine(
            settings,
            label=label,
