...
Personal search tokens are linked to a stable `user_id` and currently carry the `search:read` scope. They inherit the owner's global role and project ACLs, stop authenticating when the owner is disabled, and are accepted only by `/api/v1/search`, `/api/v1/search/batch`, `/api/v1/projects`, `/api/v1/sessions/{session_id}/turns/{turn_number}`, and that turn's `/activity` endpoint. Reusable search snapshots pin captured data, but every request rechecks current authorization; a reduced access scope invalidates the snapshot. Sync API tokens and machine credentials are not accepted by the search API.
