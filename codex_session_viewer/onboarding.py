...
def _backfill_onboarding_timestamps(connection: sqlite3.Connection) -> None:
    ...
    if not state["first_session_ingested_at"]:
        session_row = connection.execute(
            """
            SELECT s.source_host, MIN(s.imported_at) AS first_imported_at
            FROM sessions AS s
            WHERE COALESCE(s.imported_at, '') <> ''
              AND EXISTS (
                SELECT 1
            ...
            ORDER BY first_imported_at ASC
            LIMIT 1
            """
        ).fetchone()
        if session_row is not None:
            _update_onboarding_state(
                connection,
                first_session_ingested_at=trimmed(session_row["first_imported_at"]),
...
def reconcile_onboarding_state(connection: sqlite3.Connection, settings: Settings) -> dict[str, Any]:
    ...
    return {
        ...
        "health_evidence": {
            "last_heartbeat_at": trimmed(primary_remote.get("last_seen_at")) if primary_remote else state["first_heartbeat_at"],
            "source_host": trimmed(primary_remote.get("source_host")) if primary_remote else state["first_heartbeat_source_host"],
            "agent_version": trimmed(primary_remote.get("agent_version")) if primary_remote else None,
            "sync_api_version": trimmed(primary_remote.get("sync_api_version")) if primary_remote else None,
            "last_upload_count": int(primary_remote.get("last_upload_count") or 0) if primary_remote else 0,
            "last_skip_count": int(primary_remote.get("last_skip_count") or 0) if primary_remote else 0,
            "last_fail_count": int(primary_remote.get("last_fail_count") or 0) if primary_remote else 0,
            "last_error": trimmed(primary_remote.get("last_error")) if primary_remote else None,
